...
import utils_pro
from utils_pro import load_pkl, sps2csv
...
def get_data_slice(file_path, time_pkl_path = None, data_pkl_path = None, label_pkl_path = None, number = 5):
    ...
    for name in usable_list:
        ...
        time = np.array(single.iloc[:, 1])
        # 集成了原有的 get_label_pkl 函数
        label = np.array(single.iloc[0, -1])
        if pd.isnull(label):
            label = 2
        else:
            label = int(label)
...
if __name__ == '__main__':
    ...
    get_data_slice('./data/B-2022-02-23(孕14-40周检查）.csv', data_pkl_path = './temp/train_data_5.pkl',
                   time_pkl_path = './temp/train_time_5.pkl', label_pkl_path = './temp/train_label_5.pkl' ,number = 5)