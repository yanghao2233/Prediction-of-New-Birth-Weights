# -*- coding:utf-8 -*-
# @Author: yanghao
# @Create time: 2022/01/25 19:08
# @Description: T-LSTM 框架构建
...
class T_LSTM(object):
    ...
    def __init__(self, input_dim, output_dim, hidden_dim, fc_dim, train):
        ...
        self.labels = tf.compat.v1.placeholder('float', shape = [None, self.output_dim])
    ...
    def LSTM_unit(self, prev_hidden_memory, concat_input):
    ...
    def get_states(self):  # Returns all hidden states for the samples in a batch
        ...
        packed_hidden_states = tf.scan(self.LSTM_unit, concat_input, initializer=ini_state_cell, name='states')
    ...
    def get_cost_acc(self):
        ...
        cross_entropy = tf.compat.v1.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels, logits=logits))
        y_pred = tf.compat.v1.argmax(logits, 1)
        y = tf.compat.v1.argmax(self.labels, 1)
        return cross_entropy, y_pred, y, logits, self.labels

    def map_elapse_time(self, t):

        c1 = tf.constant(1, dtype=tf.compat.v1.float32)
        c2 = tf.constant(2.7183, dtype=tf.compat.v1.float32)

        # T = tf.multiply(self.wt, t) + self.bt

        T = tf.compat.v1.div(c1, tf.compat.v1.log(t + c2), name='Log_elapse_time')

        Ones = tf.compat.v1.ones([1, self.hidden_dim], dtype=tf.compat.v1.float32)

        T = tf.compat.v1.matmul(T, Ones)
